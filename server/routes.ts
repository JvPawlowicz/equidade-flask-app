    ...
    ws.on("message", async (message) => {
      try {
        ...
        if (data.type === "chat_message" && data.recipientId) {
          ...
          const recipientWs = clients.get(data.recipientId.toString());
        ...
        if (data.type === "group_message" && data.groupId) {
          ...
          const groupMembers = await db.query.chatGroupMembers.findMany({
            where: eq(chatGroupMembers.groupId, data.groupId),
          });

          // Send to all group members who are online
          for (const member of groupMembers) {
            if (member.userId.toString() !== userId) {
              const memberWs = clients.get(member.userId.toString());
              ...
              await db.insert(notifications).values({
                ...
                title: "Nova mensagem de grupo",
                content: `Nova mensagem no grupo`,
                type: "group_message",
                referenceId: newMessage.id,
                referenceType: "chat_message"
              });
